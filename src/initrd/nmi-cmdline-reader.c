...
reader_init (void)
{
	...
	reader->hash = g_hash_table_new_full (nm_str_hash, g_str_equal, g_free, g_object_unref);
	reader->array = g_ptr_array_new ();
...
reader_destroy (Reader *reader)
{
	...
	g_hash_table_unref (reader->hash);
	g_free (reader);
}

static void
add_connection (Reader *reader, const char *name, NMConnection *connection)
{
	char *name_dup;

	name_dup = g_strdup (name);
	if (g_hash_table_insert (reader->hash, name_dup, connection))
                   ...
                   NMConnectionMultiConnect multi_connect)
{
	...
	add_connection (reader, basename, connection);
